...
Note also that an output can be used only once. That's why there's a concept of "change address" in the bitcoin ecosystem: if an output of 10 EXCC is available for me to spend, but I only need to transmit 1 EXCC, I'll create a transaction with two outputs, one with 1 EXCC that I want to spend, and the other with 9 EXCC to a change address, so I can spend this 9 EXCC with another private key that I own.
...
var transaction = new Transaction()
    ...
    .to(address, amount)  // Add an output with the given amount of exels
    .change(address)      // Sets up a change address where the rest of the funds will go
    .sign(privkeySet)     // Signs all the inputs it can
```

You can obtain the input and output total amounts of the transaction in exels by accessing the fields `inputAmount` and `outputAmount`.
...
You can also override the fee estimation with another amount, specified in exels:

>>>>>>> upstream/master
```javascript
var transaction = new Transaction().fee(5430); // Minimum non-dust amount
var transaction = new Transaction().fee(1e8);  // Generous fee of 1 EXCC
```
...
* `Transaction.FEE_PER_KB`: `10000` (exels per kilobyte)
* `Transaction.FEE_SECURITY_MARGIN`: `15`
* `Transaction.DUST_AMOUNT`: `546` (exels)
