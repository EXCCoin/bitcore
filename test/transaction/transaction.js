...
describe('Transaction', function() {
  ...
  it('can serialize to a plain javascript object', function() {
    ...
    object.inputs[0].output.exels.should.equal(testAmount);
    ...
    object.outputs[0].exels.should.equal(testAmount - 10000);
  });
  ...
  describe('adding inputs', function() {
    ...
    describe('isFullySigned', function() {
      it('works for normal p2pkh', function() {
        var transaction = new Transaction()
          .from(simpleUtxoWith100000Satoshis)
          .to([{address: toAddress, exels: 50000}])
          .change(changeAddress)
          .sign(privateKey);
  ...
  describe('change address', function() {
    it('can calculate simply the output amount', function() {
      ...
      transaction.outputs[1].exels.should.equal(40000);
    ...
    it('can recalculate the change amount', function() {
      ...
      transaction.getChangeOutput().exels.should.equal(50000);
      ...
      transaction.outputs[2].exels.should.equal(30000);
    ...
    it('fee can be set up manually', function() {
      var transaction = new Transaction()
        ...
        .sign(privateKey);
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].exels.should.equal(10000);
    });
    it('fee per kb can be set up manually', function() {
      ...
      transaction.outputs.length.should.equal(2);
      transaction.outputs[1].exels.should.equal(34000);
    });
    it('if exels are invalid', function() {
      var transaction = new Transaction()
        .from(simpleUtxoWith100000Satoshis)
        .to(toAddress, 99999)
        .change(changeAddress)
        .sign(privateKey);
      transaction.outputs[0]._exels = 100;
      transaction.outputs[0]._exelsBN = new BN(101, 10);
      expect(function() {
  ...
  describe('checked serialize', function() {
    ...
    it('checks output amount before fee errors', function() {
      var transaction = new Transaction();
      transaction.from(simpleUtxoWith1EXCC);
      transaction
  ...
  describe('#verify', function() {
    ...
    it('not if _exels and _exelsBN have different values', function() {
      var tx = new Transaction()
        .from({
          'txId': testPrevTx,
          'outputIndex': 0,
      ...
      tx.outputs[0]._exels = 100;
      tx.outputs[0]._exelsBN = new BN('fffffffffffffff', 16);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 exels is invalid');
    });

    it('not if _exels is negative', function() {
      var tx = new Transaction()
        .from({
          'txId': testPrevTx,
          'outputIndex': 0,
      ...
      tx.outputs[0]._exels = -100;
      tx.outputs[0]._exelsBN = new BN(-100, 10);
      var verify = tx.verify();
      verify.should.equal('transaction txout 0 exels is invalid');
    });
  ...
  describe('removeInput and removeOutput', function() {
    it('can remove an input by index', function() {
      ...
      transaction.inputAmount.should.equal(simpleUtxoWith1EXCC.exels);
    ...
    it('can remove an input by transaction id', function() {
      ...
      transaction.inputs.length.should.equal(1);
      transaction.inputAmount.should.equal(simpleUtxoWith1EXCC.exels);
      transaction.removeInput(simpleUtxoWith1EXCC.txId, simpleUtxoWith1EXCC.outputIndex);
      transaction.inputs.length.should.equal(0);
      transaction.inputAmount.should.equal(0);
    });
    ...
    it('an output can be removed by index', function() {
      var transaction = new Transaction()
        .to([
          {address: toAddress, exels: 40000000},
          {address: toAddress, exels: 40000000}
        ])
      transaction.outputs.length.should.equal(2);
  ...
  describe('output ordering', function() {
    ...
    beforeEach(function() {
      transaction = new Transaction()
        .from(simpleUtxoWith1EXCC)
        .to([
          {address: toAddress, exels: tenth},
          {address: toAddress, exels: fourth}
        ])
        .to(toAddress, half)
        .change(changeAddress);
  ...
  describe('clearOutputs', function() {
    ...
    it('removes all outputs and maintains the transaction in order', function() {
      ...
      tx.outputs[0].exels.should.equal(10000000);
      tx.outputs[0].script.toAddress().toString().should.equal(toAddress);
      tx.outputs[1].exels.should.equal(89990000);
  ...
  describe('BIP69 Sorting', function() {
    ...
    it('sorts outputs correctly', function() {
      ...
      tx.sort();
      tx.outputs[0].exels.should.equal(1);
      tx.outputs[1].exels.should.equal(2);
      tx.outputs[2].exels.should.equal(2);
        ...
        it(inputSet.description, function() {
          ...
          inputSet.inputs = inputSet.inputs.map(function(input) {
            var input = new Input({
              ...
              script: new Script(),
              output: new Output({ script: new Script(), exels: 0 })
            });
...
var unsupportedTxObj = '{"version":1,"inputs":[{"prevTxId":"a477af6b2667c29670467e4e0728b685ee07b240235771862318e29ddbe58458","outputIndex":0,"sequenceNumber":4294967295,"script":"OP_1","output":{"exels":1020000,"script":"OP_1 OP_ADD OP_2 OP_EQUAL"}}],"outputs":[{"exels":1010000,"script":"OP_DUP OP_HASH160 20 0x7821c0a3768aa9d1a37e16cf76002aef5373f1a8 OP_EQUALVERIFY OP_CHECKSIG"}],"nLockTime":0}';
