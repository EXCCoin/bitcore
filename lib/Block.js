...
function Block(data) {
  ...
  this.extradata = data.extradata || util.EMPTY_BUFFER;
  this.stake_version = data.stake_version || 0;
  this.equihash_solution = data.equihash_solution || util.EMPTY_BUFFER;
...
Block.prototype.getHeader = function getHeader() {
  ...
  //ofs += 32
  // buf.writeUInt32LE(this.stake_version, ofs);
  put.word32le(this.stake_version);
  //ofs += 4
  // equihash 144,5 solution
  put.put(new Buffer(this.equihash_solution, 'hex'));
  //ofs += 100
...
Block.prototype.parse = function parse(parser, headerOnly) {
  ...
  this.extradata = parser.buffer(32);
  this.stake_version = parser.word32le();
  this.equihash_solution = parser.buffer(100);
...
Block.prototype.getMerkleTree = function getMerkleTree(txs) {
  ...
  for (var size = txs.length; size > 1; size = Math.floor((size + 1) / 2)) {
    ...
    for (var i = 0; i < size; i += 2) {
      ...
      var a = tree[j + i];
      var b = tree[j + i2];
      tree.push(util.sha256(Buffer.concat([a, b])));
    }
    j += size;
  }
...
Block.prototype.getStandardizedObject =
  function getStandardizedObject(txs) {
    var block = {
      ...
      stakeversion: this.stake_version,
      equihashsolution: this.equihash_solution
    };
